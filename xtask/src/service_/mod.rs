...
use llama_cu::{
    Message, Received, ReturnReason, Service, Session, SessionId, Terminal, TextBuf, utok,
};
use log::{info, warn};
use openai::{V1_CHAT_COMPLETIONS, V1_COMPLETIONS_OBJECT};
...
fn complete_chat(
    ...
    // 用于持有所有权
    let content_list: Vec<String> = messages
        .iter()
...

    {
        ...
        cache_manager_guard.send(session_id, tokens, sample_args);
    }
...
fn test_post_send() {
    ...
    let prompt = std::env::var_os("TEST_PROMPT").unwrap_or_else(|| "Once upon a time,".into());
            ...
            let req_body = serde_json::to_string(&CreateChatCompletionRequest {
                ...
                messages: vec![ChatCompletionRequestMessage::User(
                    openai_struct::ChatCompletionRequestUserMessage {
                        content: serde_json::Value::String(prompt.to_str().unwrap().to_string()),
                        name: None,
                    },
                )],
                metadata: None,
